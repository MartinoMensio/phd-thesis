...
TODO: expand on the relationships listed in the contributions above.
...
- remove oversimplification of left/right. go from this to a more statistically driven clustering analysis based on usage of language depending on the topic.
- techniques: may change over time. instead of relying on poorly trained models that only rely on a set of techniques (as in this work), derive technique analysis from difference observation of language. In this way the approach would be resistant to new techniques emerging. It would be only a matter of analysing the discovered techniques and linking them to literature to describe them.

With these improvements, the cross-dimensional analysis presented in this thesis would be able to generalise better to different context, filling the gap of "missing knowledge" from new topics / political situations (L/R) or different techniques that may emerge in a different timeframe.
