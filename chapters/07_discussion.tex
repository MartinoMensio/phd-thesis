...
We observed this change of persuasive/propagandist language by taking into consideration the simplified categorisation by \emph{political leaning} (left/center/right) and tried to characterise linguistic profiles depending on this variable.

But we observed that this variation of language is also very specific to the emph{topic}. We observed how some topics are more controversial and find a higher quantity of propaganda, while others are more neutral and find less usage of linguistic techniques.
...
After the analysis of the contributions, we can redefine some of the concepts that we used in this work. These definitions are intended to support future work as a set of revised hypotheses.
...
For evaluating the difference of intent, at the current state of automated analysis, we need to rely on human evaluation.
...
% limitations
To identify the techniques, we do not want to rely on a specific list and definition, that require dedicated data collection and annotation (see example of the propaganda corpus of~\citet{da2019fine}).
...
% how
To overcome this problem, we want to be able to discover automatically linguistic patterns that are distinctive of the political leaning of the news articles.

We can identify these patterns by observing the frequency of specific phrasing in articles and by identifying outliers. With the parallel analysis described in this work, we can more easily compare multiple articles that cover the same topic.

% redefinition
We can group them in two groups:

\begin{enumerate}
    \item linguistic patterns \emph{shared} across political orientations: this group contains linguistic patterns that are used by all political orientations.
