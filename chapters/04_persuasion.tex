...
For example, Figure~\ref{fig:example_sentiment} shows a pair of sentences where, if we only consider the output scores at the sentence level, we just have the result that the first sentence is more negative than the second one. But we could not compare the single words that are responsible for the scores.
