...
And this variety of linguistical forms is an excellent opportunity to study the relationship between the content itself (bare facts) and the additional set of words that try to persuade the reader of a certain opinion (framing, interpretation, propaganda).

% fit into thesis
But how does this fit into the overall goal of the thesis? Our research question aims at studying the relationship between propaganda and political leaning.
...
Having this first differentiation between shared, omitted and unique, we also want to assess whether this is correlated to language that wants to persuade (propaganda). 
...
The overall RQ: What is the relationship between propaganda and political leaning?
This chapter RQ: How do different sources present the same events? Can we analyse what is unique from each version and what overlaps? Is there a link between the parts that are different and propaganda/loaded language? 
...
In the following sections, we are starting in Section~\ref{sec:cgs_cross_referencing} by reproducing an experiment from a paper~\cite{bountouridis2018explaining} that automatically extracts corroborated and omitted information in groups of articles. Then in Section~\ref{sec:cgs_similarity} we explore more the methods
 to compute document similarity, then in Section~\ref{sec:cgs_clustering_and_differences}
...
\section{Paper reproduction~\cite{bountouridis2018explaining}}
...
\section{Formalisation and dissemination}
% what
