...
\begin{itemize}
    ...
    \item the best result of \texttt{Media-Balanced} \texttt{(0)+(1)+(2)+(3b)} has a way bigger delta from \texttt{(0)}: 0.3897\% F1, 0.5874\% Accuracy;
...
\begin{itemize}
    ...
    \item[H2:] propaganda is related to populism. We need to formulate and verify this hypothesis to then prove or confute H1.
\end{itemize}
...
Note that the datasets that we consider are not the datasets used for political leaning that have been described in the previous Section~\ref{ssec:ps_leaning_data}.
...
\subsubsection{\statusorange Propaganda and Populism across all political contexts}
...
\begin{itemize}
    ...
    \item United States: here, most of the propaganda is linked to the Trump-Clinton election campaigns of 2016, and then propaganda from Trump's presidency. The authors describe mostly Right-leaning propaganda.
    \item China: \todo{FINISH HERE}
\end{itemize}

Across these examples, we see that most of the propaganda is Right-wing, but we also have examples of Left-wing propaganda (Ukrainian patriotism, Brazilian Lulism).
...
And then we find other occurrences, that mention populism examples.
From~\cite{muller2017populism}:
- right populism: Marine Le Pen (France), Geert Wilders (The Netherlands)
- left populism: Bernie Sanders (US) and not only Trump (right)~\cite{postel2016if,jensen2017populism,busby2019framing} \url{https://www.oah.org/tah/issues/2016/february/if-trump-and-sanders-are-both-populists-what-does-populist-mean/}, Syriza left-wing alliance (in power January 2015 Greece), Podemos in Spain (shares with Syriza the opposition to Merkel's authority policies, Latin America ``pink tide": Raphael Correa (Ecuador), Evo Morales Bolivia), Hugo Chávez (Venezuela)
