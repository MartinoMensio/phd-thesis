...
Having in the previous Chapter covered persuasion and propaganda, we demonstrated the need to understand the context around the source of an article in order to interpret its persuasion means.

For this reason, this chapter introduces the factor of \emph{political leaning}.
...
The difference of terminology (persuasion $\rightarrow$ propaganda) between the first RQ and the second RQ is due to the exclusion of sentiment when we reply to the first RQ, therefore we only indicate propaganda from there onwards.
...
\subsection{\statusorange Political Leaning Definition}
...
As can be seen from the \href{https://www.allsides.com/media-bias/ratings}{Media Bias Ratings} published on their website, they have leaning ratings for different categories: Author (500), News Media (894), Think Tank / Policy Group (108), Reference (25) and Fact-Check (20).
...
This collection contains 7,766 headlines (as of 15/02/2023), for a total of 23,298 links to articles. The oldest headlines were published in 2012.
...
For utilising this dataset directly, however, there is a limitation. The full text of the articles needs to be separately retrieved by connecting to the corresponding news outlets, and therefore the process of collecting and cleaning the data is not error-free.
...
% in short:
For this experiment, the setup is quite straightforward. We want to see if there are some differences in the persuasion used by different political leanings. Therefore, we take a dataset, we run it through automated persuasion annotation tools, and then we compare the statistics across political leaning of the articles.
...
Then, we also have a last reason. %Both political leaning classification and propaganda analysis are types of political analysis.
...
\begin{table}[!htbp]
  ...
  %\small
    ...
    \begin{tabular}{l|rr|rr}
        ...
        \hline
        (0) + (1) & 55.7786 & 57.7692 & \textbf{42.1847} & \textbf{47.5384} \\
        (0) + (2) & 54.9714 & 56.9999 & \textbf{42.4362} & \textbf{$\star$47.6153} \\
        (1) + (2) & 36.5217 & 40.4615 & 33.6202 & 41.0000 \\
        (0) + (1) + (2) & 55.2121 & 57.1538 & \textbf{42.2102} & \textbf{47.3846} \\
        (0) + (3a) & 54.6287 & 56.6923 & \textbf{41.9766} & \textbf{46.5384} \\
        (0) + (3b) & 54.8004 & 56.3076 & \textbf{41.9965} & 46.2307 \\
        (3a) + (3b) & 44.3506 & 46.3076 & \textbf{41.8725} & 44.3076 \\
        (0) + (3a) + (3b) & 53.2549 & 54.7692 & \textbf{42.0576} & 45.92307 \\
        (1) + (2) + (3a) & 42.0611 & 43.8461 & \textbf{42.5401} & 44.5384 \\
        (0) + (1) + (2) + (3a) & 54.0515 & 56.0769 & \textbf{42.2358} & \textbf{46.8461} \\
        (1) + (2) + (3b) & 44.3166 & 45.6153 & \textbf{41.9613} & 44.0000 \\
        (0) + (1) + (2) + (3b) & 45.6153 & 45.6153 & \textbf{41.9292} & 46.2307 \\
        (1) + (2) + (3a) + (3b) & 45.0456 & 47.2307 & \textbf{$\star$43.4564} & 45.9230 \\
        (0) + (1) + (2) + (3a) + (3b) & 53.6459 & 53.6459 & \textbf{42.5509} & \textbf{46.5384} \\
...
With the current analysis, we can say that \textbf{the answer to RQ2.3 is a partial Yes: using as feature the terms of propaganda achieves a very similar score to the baseline (rows indicated with \texttt{(3a)} and \texttt{(3b)}), and when added to the baseline features, it helps to achieve a small (but not significant, \textit{p-value=0.657}) improvement}. % NOT SIGNIFICANT IMPROVEMENT: p-value=0.657


\subsubsection{\statusorange Reproduction on \texttt{NELA} dataset}
...
We have seen in the previous sections that the results of the classifier based on the propaganda features are not setting a big difference in the F1 scores. On the media splits, we managed to increase by a few percentage points, while on the random splits the baseline still provides the best results.
...
\begin{table}[!htbp]
  ...
  %\small
    ...
    \begin{tabular}{l|rr|rr|rr|rr}
        ...
        % \texttt{Prop-words-BERT} (3) & 35.4718 & 48.2387 & 35.4718 & 48.2387 \\
        \hline
        (0) + (1) & \textbf{69.2085} & \textbf{71.2271} & \textbf{70.9899} & \textbf{70.9057} & 45.1531 & \textbf{50.6024} & \textbf{46.2288} & \textbf{46.0346}  \\
        (0) + (2) & \textbf{69.2905} & \textbf{71.2991} & \textbf{71.0331} & \textbf{70.9510} & \textbf{45.1835} & \textbf{50.6412} & \textbf{46.3481} & \textbf{46.1550} \\
        (1) + (2) & 25.7629 & 47.8299 & 38.5816 & 39.8064 & 24.4286 & 46.2459 & 36.2956 & 37.1280 \\
        (0) + (1) + (2) & \textbf{69.2994} & \textbf{71.3019} & \textbf{$\star$71.0358} & \textbf{$\star$70.9529} & \textbf{45.1998} & \textbf{50.6524} & \textbf{46.3457} & \textbf{46.1517} \\
        (0) + (3a) & 65.7886 & 68.5003 & 68.5460 & 68.4628 & 44.7002 & 49.4820 & 46.1694 & 46.1916 \\
        (0) + (3b) & 66.1823 & 68.7194 & 68.5074 & 68.4349 & 45.0300 & 49.8181 & \textbf{46.4617} & \textbf{46.4431} \\
        (3a) + (3b) & 41.4558 & 49.5529 & 42.5407 & 42.6677 & 37.0566 & 45.1370 & 36.5396 & 36.5419 \\
        (0) + (3a) + (3b) & 65.0796 & 67.8463 & 67.4347 & 67.3705 & & & 45.9924 & 46.0147  \\
        (1) + (2) + (3a) & 41.6843 & 49.9994 & 42.2068 & 42.7608 & & & 36.4236 & 36.4208   \\
        (0) + (1) + (2) + (3a) & 65.7779 & 68.4993 & 68.5715 & 68.4921 & & & \textbf{46.2871} & \textbf{46.3227}  \\
        (1) + (2) + (3b) & 38.8414 & 50.4508 & 39.6869 & 41.3132 & & & 35.8977 & 36.0895  \\
        (0) + (1) + (2) + (3b) & 66.1066 & 68.6778 & 68.5074 & 68.4376 & & & \textbf{$\star$46.6062} & \textbf{$\star$46.6054} \\
        (1) + (2) + (3a) + (3b) & & & 42.6323 & 42.7555 & & & 36.7762 & 36.8838 \\
        (0) + (1) + (2) + (3a) + (3b) & & & 67.4353 & 67.3718 & & & 46.05313 & 46.0858  \\
...
\paragraph{Metrics/Scores Analysis}
...
\todo{Finish when all results are there}

% balanced vs imbalanced
Balanced and imbalanced datasets have very similar results.
...
When tested for statistical significance with the McNemar test, 


But also in this case, even if the results are statistically significant, the margins are very narrow.

\paragraph{What the models learn}

Analysis of what the models learned

confusion matrix analysis (where is the error)

weights analysis: is it similar?


% features: ['random'] splits: random balanced: unbalanced
% Macro-F1: 21.56329772147112
% Accuracy: 47.808618792251124
% Flip Error-rate: 34.68653379216378
% MAE: 0.8687791499991266
% features: ['bert'] splits: random balanced: unbalanced
% Macro-F1: 69.20762604343277
% Accuracy: 71.22119936415882
% Flip Error-rate: 16.809265114329136
% MAE: 0.45588065750170315
...
% Limitations
\noindent\textbf{Imbalance in propaganda detection}: One limitation is that only articles from the Right political spectrum were used for training the propaganda detection tool provided in \cite{da2019fine}. %, which we use in our analysis. 
...
This work analysed the relationship between propaganda and political leaning detection. Using available works for propaganda detection and political leaning, we explored the effects of the propaganda features on the task of political leaning classification. We showed that (a) propaganda is equally found in Left, Centre, and Right leaning articles, (b) detecting political-leanings with propaganda features is challenging, and (c) detection performance improves when we use the amount of each propaganda technique together with the baseline features (0)+(2).
...
\begin{enumerate}
    ...
    \item Correlation between propaganda and populism: here we test our hypothesis with correlation analysis. With a dataset that contains speeches with leaning and populism information, we analyse the speeches with propaganda analysis to see the correlation between the detected propaganda techniques against the populism annotations. We do a breakdown by leaning and technique to see where the correlation is stronger.
...
\subsubsection{\statusgreen The correlation between Populism and Propaganda}
...
\section{\statusorange Discussions}
\label{sec:ps_discussions}

what was achieved, findings by RQ:

\todo{take findings for each experiment, put them together and discuss}
