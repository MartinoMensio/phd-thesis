...
In this chapter, we present the state of the art for the different research areas that we plan to use in the next chapters. Each of the sections is related to one of the Research Questions presented in the previous chapter.
...
\section{\statusorange Similarity between articles}
\label{sec:lit_relationships}

Our starting point is to analyse the literature that aims at analysing multiple versions of the same story, across different news sources.
...
We are interested in works that explore what changes between them, and the literature defines and deals with different tasks (Subsection~\ref{ssec:lit_relationships_tasks}). We focus specifically on corroboration and omission detection (Subsection~\ref{ssec:lit_relationships_corr_omiss}.
...
Hand-curated vs algorithmic datasets

\subsection{Common Tasks}
\label{ssec:lit_relationships_tasks}

Semantic similarity task: benchmarks~\citep{conneau-kiela-2018-senteval,chandrasekaran2021evolution}.
Models:
GloVe word embeddings~\citep{pennington2014glove}
BERT most popular embeddings provided by Google Research~\citep{devlin2018bert}
\acrfull{use}~\citep{cer2018universal}
XLNet~\citep{yang2019xlnet}

Applied to finding related claims~\citep{almeida2020text}
...
Investigation across multiple sources (corroboration vs omission)
...
- \citet{bountouridis2018explaining} to analyse corroboration and omission
- \citet{ko2023claimdiff} analyses the interrelationship between points of view, dataset derived from AllSides.
...
\section{\statusred Persuasion}
\label{sec:lit_persuasion}

\todo{Continue here to take all citations from Chapters 4-6 and expand on them}
