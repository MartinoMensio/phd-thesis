...
For the detected persuasion, we have that in $89.8\%$ of the sentences, some techniques are detected. And it is always sentiment that gets triggered, while propaganda is detected only in $14\%$ of the sentences.
The average percentage of words that are detected as related to any persuasion technique is $10.1\%$, meaning that even though persuasion appears in approximately 9 out of 10 sentences, usually it appears just in a few words. Breaking down by type of technique, most of the words are detected as carrying sentiment ($12.5\%$) while only $3.2\%$ related to propaganda.
We can see that the percentages are much smaller than the values provided by the sentiment libraries.
...
We analyse the ratio between the average magnitude of changes from this table and the average word-based percentage from Table~\ref{tab:detected_persuasion_in_variations} to normalise the results and find the techniques that are mostly affected by this variation.
...
While all these results are really useful to understand the relationship between the detected persuasion and the variations appearing in multiple versions of the same detail, we have a main limitation which is: does this analysis reflect on what a news consumer would perceive?
...
\todo{Rephrase and finish}
% findings
% quantitative
Sentiment detection might not be the best choice. Very noisy detection, picking up lots of terms.
When normalisation is performed, we find that \texttt{Loaded\_Language} being the easiest technique to deploy. More subtle techniques instead appear to have a higher normalised score because they only appear in one of the two sentences in the pairs analysed.
This is particularly meaningful because TODO
% qualitative
