...
also used in the previous Chapter~\ref{chap:common_ground_search}: we use for comparison both TF-IDF and \Gls{use}. The first one to represent simpler approaches while the second one to stand for more recent and complex ones. The aim of the comparison is to understand whether there is any advantage of using more sophisticated approaches for this task.
...
%: firstly, related to increasing the distance threshold in hierarchical clustering, then 


% Step 4 results (curve)
\paragraph{Hierarchical Agglomerative Clustering}
...
or we can compare the full curve. For simplicity, we compare the maximum value of the curve, reporting also the distance where it has been reached. However, the full curve has analogue results, as seen in the figure above (higher maximum point, higher curve).

Still in Figure~\ref{fig:hierarchical_sentpropnoise_evolution} we can see that removing propaganda helps TF-IDF reaching 100\% perfect clustering with threshold 1.26 euclidean distance (12 articles on 3 topics: Supreme court, Elections, Coronavirus, Supreme Court). Instead without removal, TF-IDF misplaces some elements with the full text.


% table: step 5 results? Check
\paragraph{K-Means}
For evaluating the effects with different sizes f datasets, we show here some results with K-Means.
...
10 & 30 & 6 & \multicolumn{1}{l|}{\begin{tabular}[c]{@{}l@{}}.616R\\ .765M\end{tabular}} & \multicolumn{1}{l|}{\begin{tabular}[c]{@{}l@{}}.616R\\ .765M\end{tabular}} & \multicolumn{1}{l|}{\begin{tabular}[c]{@{}l@{}}.616R\\ .765M\end{tabular}} & \begin{tabular}[c]{@{}l@{}}.616R\\ .765M\end{tabular} & \multicolumn{1}{l|}{\begin{tabular}[c]{@{}l@{}}.547R\\ .724M\end{tabular}} & \multicolumn{1}{l|}{\begin{tabular}[c]{@{}l@{}}.547R\\ .724M\end{tabular}} & \multicolumn{1}{l|}{\begin{tabular}[c]{@{}l@{}}.547R\\ .724M\end{tabular}} & \begin{tabular}[c]{@{}l@{}}.547R\\ .724M\end{tabular} \\ \hline
...
\end{tabular}%
 ...
 \caption{Results of the clustering applied to USE features with K-Means. The explanation is the same as in Table~\ref{tab:sentpropnoise_tfidf}. The difference is that the vector representations are derived}
...
We can notice that in the first row, with only two headlines, the metrics are seeing that the evaluation clusters are identical to the reference clusters (score of $1.0$ both ARI and AMI).

Then in the second row, we can see that also here (as previously in Figure~\ref{fig:hierarchical_sentpropnoise_evolution}, that with topic clustering we reach $1.0$ with AMI and ARI, because we manage to match perfectly with the ground truth clusters.
...
and fourth row, we see that TODO

USE results:
propaganda removed does not help, even with small dataset. Propaganda pieces in an article are usually more than isolated words, are spans with longer length. Instead adjectives are usually isolated words. USE with respect to TF-IDF is more exposed to the sequence of words, and by inspecting the ``cleaned" articles we can see that the sentences are incomplete and cannot stand. For this reason, a simpler model like TF-IDF is being helped by the removal of propaganda, while actually for USE the sentences become broken and the clustering struggles more.

Discrepancy between AMI and ARI: TODO

\paragraph{Inspection of the predictions}
Let’s consider for inspection the articles belonging to cluster 0 and cluster 7.
...
Explanation:
Terms like “win” (win, winner, ..) are removed and more subtle changes in terms usage is possible:
Lead in cluster 0 (5th November)
Patience, legal, wait in cluster 7 (3 November)

The removal of these terms helps in the fine-grained differentiation of the stories but may not help with coarse topic clustering


\paragraph{Limitations and findings}
...
Limitation: when increasing the size, difference becomes smaller and smaller. This means that ???

Limitation 2: removing terms from articles makes them incorrect grammatically. For TF-IDF encoding, this is not a problem. But for USE it could be a problem.

Limitation 3: if some words are both persuasion and topical, by removing them, it becomes more difficult to form the correct cluster. This is why non-persuasive rephrasing could be needed. (choice of terms, but still some meaning).
