...
We start with comparing the original motivations with the actual findings in Section~\ref{sec:discussion_goals}. Then Section~\ref{sec:discussion_findings} presents our main contribution (granularly by \acrlong{rq}[s]) and Section~\ref{sec:discussion_findings_implications} the implications that the findings may have.
...
\section{Comparing with our Motivations}
...
\begin{enumerate} % computational point of view
    ...
    \item Motivation: improve some automated approaches that currently rely on just one input feature, such as political leaning classification using propaganda and topic features.
    ...
    \item Motivation: discover problems and inconsistencies that can only be discovered with combined analyses (e.g., data imbalance). %This is a very important point when using \acrshort{ml}-based approaches, as improving the quality of the data is a key element. % contribution 3: 
...
\section{Anwers to the Research Questions}
...
\begin{enumerate}[label={\textbf{RQ1.\arabic*:}},leftmargin=2cm]
    ...
    which is based on splitting the articles into sentences and comparing with similarity metrics. We extended the approach to analyse at the word level the variation of terms. %(Section~\ref{sec:cgs_clustering_and_differences}).
...
\section{Contributions/Findings}
% \section{OVERALL CONTRIBUTION / Findings}
\label{sec:discussion_findings}

The main contribution of this work is to bring more clarity about the relationship between propaganda, similarities across news articles, political leaning and topics. The most important ones come from the last Chapters~\ref{chap:political_sides} and~\ref{chap:topics} where we identify the data imbalance of the propaganda datasets and where we identify topics where the propaganda differs mostly between left and right.
...
For example, we find that for some topics there exists more propaganda specifically on one side of the political spectrum: economy, labour, disasters, court. In some other cases instead, propaganda appears uniformly across the spectrum (Fundamental Rights and Religion) but when we consider the terms we have significant differences (low correlation).
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Chapter~\ref{chap:political_sides} showed, by improving the classifier values of F1 metric, that propaganda and political leanings are slightly correlated. If they were not correlated, adding propaganda as input feature would have no effect.
        \item Chapter~\ref{chap:topics} found that the topic is highly correlated with propaganda: some topics have very high levels of propaganda, while other ones are less polarizing and contain less of it. And considering the terms of propaganda, we find that for some topics they are very different between political leanings. %This not only means that the correlation between topics, leanings and propaganda is high, but it also means that
...
\section{Implications of the findings and limitations}
...
We are eager to explore additional research avenues based on the findings presented in this doctoral thesis and are committed to enhancing the methodologies for greater scientific accuracy.